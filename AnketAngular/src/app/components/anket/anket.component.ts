...
export class AnketComponent implements OnInit {
  ...
  AnketCoz(Ankets : Anket){
    if(JSON.parse(localStorage.getItem('anketCozId')) == null){
      localStorage.setItem('anketCozId', JSON.stringify(Ankets.AnketId))
    }
    else{
      var a = JSON.parse(localStorage.getItem('anketCozId'));
      a = Ankets.AnketId;
      localStorage.setItem("anketCozId",JSON.stringify(a));
    }
    this.apiServis.ikinciAnket(JSON.parse(localStorage.getItem("anketCozId"))).subscribe((d : Sonuc)=>{
      if(d.islem == false){
        this.alert.AlertUygula(d)

      }
      else{
        location.href="/anketkullan";
      }
    })
